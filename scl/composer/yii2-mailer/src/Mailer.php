...
namespace scl\mailer;

use scl\mailer\job\SendEmailJob;
use Yii;
...
class Mailer extends BaseMailer
{
     ...
     */
    public $messageClass = 'scl\mailer\Message';
    /**
    ...
    public function init()
    {
        ...
        if (!empty($this->config)) {
            ...
            $this->adapter->setCallback(ArrayHelper::remove($this->config, 'callback',
                'scl\mailer\Mailer::processResult'));

            //special handling of charset. Note: PHPMailer in [[createBody()]] overrides charset and sets 'us-ascii' if no 8-bit chars are found!
            $this->adapter->setCharset(!empty($this->messageConfig['charset']) ? $this->messageConfig['charset'] : ArrayHelper::remove($this->config,
    ...
    private function getComponentName()
    {
        ...
        foreach (Yii::$app->getComponents() as $id => $component) {
            ...
            if ($this instanceof $component['class'] && $this === Yii::$app->get($id)) {
                $this->_componentName = $id;
                return $this->_componentName;
            }
        }
    }
    ...
    protected function sendMessage($message)
    {
        if ($this->useQueue) {
            try {
                if (!$this->adapter->preSend()) {
                    return false;
                }

                $this->queue->push(new SendEmailJob(
                    $this->adapter->serializeParams(),
                    $this->logCategory,
                    $this->getComponentName()
                ));
            } catch (\Exception $e) {
                Yii::error('FAILED - Sending email: ' . $e->getMessage(), $this->logCategory);
                return false;
            }
            return true;
        }
        ...
        if ($result === false) {
            Yii::warning([
                ...
                'info' => implode(';', array_keys($message->getTo())) . ' "' . $message->getSubject() . '"',
